...
import { z } from 'zod';
import {
  ALERT_MERGE_BULK_PATH,
  type TheHiveCase,
  type TheHiveCortexJob,
  type TheHiveObservable,
  alertToCasePath,
  buildMergeBulkBody,
  joinUrl,
} from './queries.js';
 ...
 *
 * Generic over the response type (finding 25): callers hitting a stable
 * endpoint pass the matching interface (e.g. `thehiveApi<TheHiveCase>(...)`) for
 * field-level type safety; the generic /api/v1/query endpoint keeps the default
 * `unknown`. The cast is a documentation/extraction aid only — there is no
 * runtime schema validation, so the index-signatured interfaces let unmodeled
 * fields pass through formatResponse untouched.
 */
async function thehiveApi<T = unknown>(method: string, path: string, body?: unknown): Promise<T> {
...
server.tool(
  ...
  toolGuard(
    SERVER_NAME,
    'create-case',
    async ({ title, description, severity, tags, tlp, pap }) => {
      auditWrite('create-case', { title, severity, tags, tlp, pap });
      const result = await thehiveApi<TheHiveCase>('POST', '/api/v1/case', {
        title,
        description,
        severity,
        tags,
...
server.tool(
  ...
  toolGuard(SERVER_NAME, 'get-case', async ({ case_id }) => {
    ...
    const caseData = await thehiveApi<TheHiveCase>('GET', `/api/v1/case/${case_id}`);
...
server.tool(
  ...
  toolGuard(
    SERVER_NAME,
    'add-observable',
    async ({ case_id, data_type, data, tags, message, ioc, tlp, pap }) => {
      auditWrite('add-observable', { case_id, data_type, tags, ioc, tlp, pap });
      const result = await thehiveApi<TheHiveObservable>(
        'POST',
        `/api/v1/case/${case_id}/observable`,
        {
          dataType: data_type,
          data,
          tags,
...
server.tool(
  ...
  toolGuard(SERVER_NAME, 'run-analyzer', async ({ observable_id, analyzer_id }) => {
    ...
    const result = await thehiveApi<TheHiveCortexJob>('POST', '/api/v1/connector/cortex/job', {
...
server.tool(
  ...
  toolGuard(SERVER_NAME, 'get-analyzer-report', async ({ job_id }) => {
    const result = await thehiveApi<TheHiveCortexJob>(
      'GET',
      `/api/v1/connector/cortex/job/${job_id}`,
    );
    return { content: [{ type: 'text' as const, text: formatResponse(result) }] };
  }),
);
...
server.tool(
  ...
  '[WRITE] Merge related TheHive alerts into a single case. If no case_id is given, a new case is created from the first alert and the rest are merged into it. This permanently combines alerts.',
  {
    ...
    case_id: safeId
      .optional()
      .describe(
        'Existing case ID to merge into (new case created from the first alert if omitted)',
      ),
  },
  ...
  toolGuard(SERVER_NAME, 'merge-alerts', async ({ alert_ids, case_id }) => {
    ...
    if (!targetCaseId) {
      ...
      // Typed response (finding 25): the promotion returns a case document, so
      // its id is reachable as _id (TheHive 5) or id without an inline cast.
      const promoted = await thehiveApi<TheHiveCase>('POST', alertToCasePath(first), {});
      promotedCase = promoted;
      targetCaseId = promoted._id ?? promoted.id;
      toMerge = rest;
    ...
    if (toMerge.length > 0) {
      merge = await thehiveApi(
        'POST',
        ALERT_MERGE_BULK_PATH,
        buildMergeBulkBody(targetCaseId, toMerge),
      );
    }
